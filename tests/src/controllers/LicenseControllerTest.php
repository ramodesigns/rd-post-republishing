...
class LicenseControllerTest extends TestCase
{
    ...
    public function test_register_rest_routes()
    {
        ...
        // We expect 6 routes to be registered now (retrieve, save, clear, retrievepublic, savepublic, clearpublic)
    ...
    /**
     * Test handle_clear_license_request success
     */
    public function test_handle_clear_license_request_success()
    {
        $auth_helper_mock = Mockery::mock('Authorisation_Helper');
        $controller = new License_Controller($auth_helper_mock);
        
        $license_service_mock = Mockery::mock('License_Service');
        $license_service_mock->shouldReceive('clear_license')->andReturn(true);
        ...
        $request = new \WP_REST_Request('POST', '/postmetadata/v1/license/clear');
        $response = $controller->handle_clear_license_request($request);
    ...
    /**
     * Test handle_clear_license_request failure
     */
    public function test_handle_clear_license_request_failure()
    {
        $auth_helper_mock = Mockery::mock('Authorisation_Helper');
        $controller = new License_Controller($auth_helper_mock);
        
        $license_service_mock = Mockery::mock('License_Service');
        $license_service_mock->shouldReceive('clear_license')->andReturn(false);
